...
To adjust word timings, music sections, and storyboard shots:
```bash
...
After editing the `_alignment.json`, refresh the `.ass` file either by clicking "重新生成 ASS" in the Web Editor, or running:
```bash
python -m src.main -i ./input/song.wav -o ./output --ass-only
```
